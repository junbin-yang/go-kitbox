...
func beegoHandlerWrite(ctx *beegocontext.Context) {
	ctx.WriteString(ctx.Input.Param(":name"))
}

func beegoHandlerTest(ctx *beegocontext.Context) {
...
// Zallocrout
func zallocroutHandler(ctx context.Context) error {
	return nil
}

func zallocroutHandlerWrite(ctx context.Context) error {
	w := ctx.Value("http.ResponseWriter").(http.ResponseWriter)
	name, _ := zallocrout.GetParam(ctx, "name")
	io.WriteString(w, name)
	return nil
}

func zallocroutHandlerTest(ctx context.Context) error {
	w := ctx.Value("http.ResponseWriter").(http.ResponseWriter)
...
// zallocroutHTTPAdapter 适配 zallocrout 到 http.Handler
type zallocroutHTTPAdapter struct {
	router *zallocrout.Router
}

func (a *zallocroutHTTPAdapter) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	ctx, handler, middlewares, ok := a.router.Match(r.Method, r.URL.Path, r.Context())
	if !ok {
		http.NotFound(w, r)
		return
	}
...
func loadZallocrout(routes []route) http.Handler {
	h := zallocroutHandler
	if loadTestHandler {
		h = zallocroutHandlerTest
	}
	...
	return &zallocroutHTTPAdapter{router: router}
}

func loadZallocroutSingle(method, path string, handler zallocrout.HandlerFunc) http.Handler {
	router := zallocrout.NewRouter()
	router.AddRoute(method, path, handler)
	return &zallocroutHTTPAdapter{router: router}
}

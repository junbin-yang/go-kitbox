...
// ResetTimer 重置定时器的触发间隔
// 参数:
//   id: 定时器ID
//   newInterval: 新的触发间隔或延迟时间
// 返回: 若ID不存在则返回错误，否则返回nil
...
//   id: 定时器ID
// 返回: 定时器信息和是否存在的标志
func (m *Manager) GetTimer(id string) (*TimerInfo, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// ListTimers 获取所有定时器ID列表
func (m *Manager) ListTimers() []string {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// safeCallback 安全执行回调函数，捕获panic
func (m *Manager) safeCallback(callback func()) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Printf("[TIMER] callback panic: %v\n", r)
		}
	}()
	callback()
}

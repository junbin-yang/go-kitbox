...
func NewRotateByTime(cfg *RotateConfig) io.Writer {
	...
	// 安全处理文件名，支持无扩展名的文件
	pattern := cfg.Filename + ".%Y-%m-%d-%H-%M-%S"
	if idx := strings.LastIndex(cfg.Filename, "."); idx != -1 {
		pattern = cfg.Filename[:idx] + ".%Y-%m-%d-%H-%M-%S" + cfg.Filename[idx:]
	}

	l, err := rotatelogs.New(pattern, opts...)
	if err != nil {
		panic("failed to create rotate logger: " + err.Error())
	}
	return l
}

...
type Connection struct {
	...
	congestionWnd uint32 // 拥塞窗口大小（内置算法使用）
	ssthresh      uint32 // 慢启动阈值（内置算法使用）

	// 拥塞控制（可选外部算法）
	useExternalCC bool                  // 是否使用外部拥塞控制
	ccController  congestion.Controller // 外部拥塞控制器
...
func NewConnection(localAddr, remoteAddr net.Addr) (*Connection, error) {
	...
	conn := &Connection{
		...
		delayedAckDur: 40 * time.Millisecond, // 延迟ACK 40ms
		ctx:           ctx,
		cancel:        cancel,
...
func (c *Connection) Connect() error {
	...
	case ackSeq := <-c.ackChan:
		...
		c.logger.Debugf("Client connected: local=%s remote=%s", c.localAddr.String(), c.remoteAddr.String())
		return nil
	case <-timeout.C:
		c.Close()
...
func (c *Connection) Listen() error {
	...
	c.logger.Debugf("Server listening: local=%s", c.localAddr.String())
	...
	case <-c.listenChan:
		atomic.StoreInt32(&c.state, StateConnected)
		c.startTime = time.Now()
		c.logger.Debugf("Server accepted connection: remote=%s", c.remoteAddr.String())
		return nil
...
func (c *Connection) Close() error {
	...
	c.logger.Debugf("Connection closed: bytesSent=%d bytesReceived=%d", c.stats.BytesSent, c.stats.BytesReceived)
...
func (c *Connection) SetFlowControl(window uint32) {
	...
	c.sendWindow = window
	c.logger.Debugf("Flow control window updated: window=%d", window)
}
...
func (c *Connection) GetStatistics() ConnectionStats {
	...
	if !c.startTime.IsZero() {
		...
		if duration > 0 {
			stats.Bandwidth = uint64(float64(stats.BytesSent) / duration)
		}
	}

	// 计算丢包率
	if stats.PacketsSent > 0 {
		stats.PacketLoss = float64(stats.Retransmissions) / float64(stats.PacketsSent)
	}
...
func (c *Connection) sendPacket(packet *Packet) error {
	...
	if packet.Type == PacketTypeData {
		c.logger.Debugf("TX Data: seq=%d len=%d", packet.Sequence, len(packet.Data))
		if len(packet.Data) == 0 {
			c.logger.Warnf("Skip sending empty data packet: seq=%d", packet.Sequence)
			return nil
		}
	}
	...
	// 通知拥塞控制：数据包已发送
	if packet.Type == PacketTypeData {
		c.onPacketSent(len(packet.Data))
	}
	...
	for {
		...
		default:
			...
			if atomic.LoadInt32(&c.state) == StateConnected &&
				...
				c.logger.Warnf("Received packet from unknown address: expected=%s received=%s", c.remoteAddr.String(), remoteAddr.String())
				continue
			}
...
func (c *Connection) processPacket(packet *Packet) {
	...
	default:
		c.logger.Warnf("Unknown packet type: type=%d", packet.Type)
	}
}

// handleDataPacket 处理数据报文
func (c *Connection) handleDataPacket(packet *Packet) {
	c.logger.Debugf("RX Data: seq=%d expected=%d len=%d", packet.Sequence, c.receiveSeq, len(packet.Data))
	...
	if err := c.receiveBuffer.Write(packet.Data); err != nil {
		c.logger.Warnf("Receive buffer full: %v", err)
		return
	}
	...
	if c.sendBuffer.Readable() > 0 {
		...
		c.pendingAck = 0
	} else if c.pendingAck == 0 {
		// 第一个包：启动延迟定时器
		c.pendingAck = c.receiveSeq
		c.pendingAckTS = packet.Timestamp
		if c.ackTimer == nil {
			...
			c.ackTimer.Reset(c.delayedAckDur)
		}
	} else {
		// 第二个包：立即发送ACK
		if c.ackTimer != nil {
...
func (c *Connection) handleAckPacket(packet *Packet) {
	...
	c.logger.Debugf("RX ACK: ack=%d oldSendAck=%d dupAck=%d", packet.Ack, oldSendAck, c.dupAckCount)

	// 更新RTT（使用ACK中回显的时间戳）
	// Timestamp字段存储的是我们发送数据包时的相对时间戳（毫秒），对端在ACK中回显
	var rtt time.Duration
	if packet.Timestamp > 0 {
		// 计算发送时间：createdTime + 相对时间戳
		sendTime := c.createdTime.Add(time.Duration(packet.Timestamp) * time.Millisecond)
		now := time.Now()
		rtt = now.Sub(sendTime)
		// 只有当时间戳合理时才更新RTT（避免时间倒流或过大的RTT）
		if rtt > 0 && rtt < 60*time.Second {
			c.updateRTT(rtt)
			c.logger.Debugf("RTT updated: ts=%d rtt=%v", packet.Timestamp, rtt)
		}
	}
	...
	if packet.Ack > c.sendAck {
		...
		// 通知拥塞控制：收到ACK
		if rtt > 0 && rtt < 60*time.Second {
			c.onAckReceived(int(packet.Ack-oldSendAck), rtt)
		} else {
...
func (c *Connection) handleWindowUpdate(packet *Packet) {
	...
	c.logger.Debugf("Received window update: new window=%d", packet.Window)
}
...
func (c *Connection) sendPendingData() {
	...
	for {
		...
		if err != nil {
			c.logger.Errorf("Failed to read send buffer: %v", err)
			break
		}
		// 不要发送空数据片段
		if len(data) == 0 {
		...
		if err := c.sendPacket(packet); err != nil {
			c.logger.Errorf("Failed to send data packet: %v", err)
			// 数据放回缓冲区
...
func (c *Connection) checkRetransmissions() {
	...
	for _, p := range packets {
		...
		if p.Attempts >= MaxRetransmissions {
			c.logger.Errorf("Max retransmissions reached, closing connection: sequence=%d", p.Sequence)
			c.Close()
			return
		}
		...
		if err != nil {
			c.logger.Errorf("Failed to retransmit packet: sequence=%d error=%v", p.Sequence, err)
			continue
		}
		...
		// 通知拥塞控制：超时丢包（仅第一次重传时通知）
		if p.Attempts == 1 {
			c.onPacketLost()
		}

		c.logger.Debugf("Retransmitted packet: sequence=%d retries=%d", p.Sequence, p.Attempts)
	}
}
...
func (c *Connection) updateRTT(rtt time.Duration) {
	c.logger.Debugf("updateRTT called: rtt=%v srtt=%v", rtt, c.srtt)
	// 更新平滑RTT (RFC 6298)
	if c.srtt == 0 {
		// 首次测量
		c.srtt = rtt
		c.rttvar = rtt / 2
		c.logger.Debugf("First RTT measurement: srtt=%v rttvar=%v", c.srtt, c.rttvar)
	} else {
		alpha := 0.125
		beta := 0.25
...
func (c *Connection) updateCongestionWindow(ack bool) {
	if ack {
		...
		if c.ssthresh < DefaultMTU*2 {
			c.ssthresh = DefaultMTU * 2 // 保证最小阈值
		}
		c.congestionWnd = c.ssthresh // 进入快速恢复
		c.logger.Debugf("Congestion detected: ssthresh=%d cwnd=%d", c.ssthresh, c.congestionWnd)
	}
}
